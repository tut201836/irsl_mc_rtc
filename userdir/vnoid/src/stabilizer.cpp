...
void Stabilizer::CalcDcmDynamics(const Timer& timer, const Param& param, const Base& base, const vector<Foot>& foot, Vector3 theta, Vector3 omega, Centroid& centroid){
	...
	double T_mh  = T/(m*h);
	double T2_mh = T*T_mh;
	double rate = 0.0; // or some tuning parameter
	...
	// --- 坂道補正 ---
	Vector3 slope_normal = base.ori * Vector3(0.0, 0.0, 1.0);
	std::cout << "slope_normal = " << slope_normal.transpose() << std::endl;

	double slope_gain = 0.1;  // チューニング用
	Vector3 slope_correction = Vector3(
		slope_gain * slope_normal.x(),
		slope_gain * slope_normal.y(),
		0.0
	);
	
	centroid.zmp_ref = centroid.zmp_target
					 + dcm_ctrl_gain * (centroid.dcm_ref - centroid.dcm_target)
					 + rate * T * delta
					 + slope_correction;  // ← ZMPを前方補正
...
void Stabilizer::Update(const Timer& timer, const Param& param, Centroid& centroid, Base& base, vector<Foot>& foot){
	Matrix3d ori_mat = base.ori.toRotationMatrix();  // Quaternion → 回転行列に変換

	if (ori_mat.col(2).x() > 0.25) {
		centroid.dcm_target.x() += 0.001;
	
		for (int i = 0; i < 2; ++i) {
			foot[i].pos_ref.x() += 0.001;
		}
	
		centroid.zmp_target.x() += 0.001;
	}
	...
	// Rotational correction of gravity vector with 
